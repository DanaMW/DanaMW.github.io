// ==UserScript==
// @name         TweetDeckSlideAppHeader
// @version      0.0.2
// @namespace    https://192.168.0.3/jscripts/scripts/TweetDeckSlideAppHeader.user.js
// @updateURL    https://192.168.0.3/jscripts/scripts/TweetDeckSlideAppHeader.user.js
// @description  Slides the App Header in and out on hover.
// @author       Dana Meli
...
body.mouse-in .js-app-header.pin-all.app-header.is-condensed .js-app-header-inner.pin-v.app-header-inner {
...
body.mouse-out .js-app-header.pin-all.app-header.is-condensed .js-app-header-inner.pin-v.app-header-inner {
    border-radius: 0 !important;
    position: fixed;
    left: 0;
    height: 100%;
...
body.mouse-in .js-app-header.pin-all.app-header .js-app-header-inner.pin-v.app-header-inner {
    border-radius: 0 !important;
    position: fixed;
    height: 100%;
    width: 200px;
...
body.mouse-out .js-app-header.pin-all.app-header .js-app-header-inner.pin-v.app-header-inner {
    border-radius: 0 !important;
    position: fixed;
    left: 0;
    height: 100%;
...
body:not(.mouse-out) .js-app-header.pin-all.app-header,
body:not(.mouse-in) .js-app-header.pin-all.app-header,
body:not(.mouse-out) .js-app-header.pin-all.app-header.is-condensed,
body:not(.mouse-in) .js-app-header.pin-all.app-header.is-condensed {
    position: initial;
}
.btn-slide {
...
function main() {
    ...
    if (noBtn) {
        $('.pull-left.margin-l--10.margin-r--2.column-type-icon.icon.icon-home').each(function() {
            $('<button>', {
                id: 'btn-slide',
                class: 'btn-slide',
                title: 'Slides the App Header on hover.',
            ...
            }).insertAfter('.pull-left.margin-l--10.margin-r--2.column-type-icon.icon.icon-home');
            $('<style>')
                .prop('type', 'text/css')
                .html(injCSS)
                .appendTo('head');
...
$(document).on('click', '#btn-slide', (function() {
    $('body').toggleClass('mouse-out');

    if ($('body.mouse-out').length > 0) {
        $(document).on('mousemove', function(event) {
            if ($('body.mouse-out').length > 0 && event.pageX < 40) {
                $('body').toggleClass('mouse-in');
            }
        })
    };
    if ($('body.mouse-in').length > 0) {
        $(document).on('mousemove', function(event) {
            if ($('body.mouse-in').length > 0 && event.pageX > $(window).width() - 205) {
                $('body').toggleClass('mouse-out');
            }
        });
    } else {
