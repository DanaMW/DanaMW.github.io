...
var style_autoplay = "\
ytd-compact-autoplay-renderer #head {display:none !important;}\
ytd-compact-autoplay-renderer {border-bottom:0px !important; margin-bottom:0px !important; padding-bottom:0px !important;}\
"
...
/* options */\
#ytpc_options_popup {direction:ltr; position:absolute; top:0px; right:0px; width:235px; box-shadow:0px 0px 6px 2px gray; font-size:11px; color:black; background:white; padding:5px; border-radius:5px; z-index:10;}\
...
function close_ytplayer_options(e) {
    ...
    for (var i = 0; i < 5; i++) {
        if (p) {
            if (p.id)
                if (p.id.search(/ytpc|gridtube/) == 0) {
                    e.stopPropagation();
                    return;
                }
            if (p.className)
                if (p.className.search(/ytpc|gridtube/) == 0) {
                    e.stopPropagation();
                    return;
                }
        }
...
function build_yt_control() {
    ...
    if (!node) {
        ...
        node = newNode("span", "ytpc_ytcontrol_container", null, parent);
        if (!node) return;
    ...
    //loop button
    if (node) {
        var loop = doc.getElementById("ytpc_ytcontrol_loop");
        if (get_pref("ytLButton")) {
            if (!loop) {
                loop = newNode("span", "ytpc_ytcontrol_loop", null, node);
...
function cinema(start_count) {
    ...
    var ratio = 16 / 9; //max width/height aspect ratio of visible area
    var pratio = ratio; //video aspect ratio
    ...
    if (pl) {
        ...
        pratio = pwidth / pheight; //video aspect ratio
        if (pratio < ratio) ratio = pratio; //adopt video aspect ratio for narrower videos
    }

    var height = H - (hide ? 0 : 56); //visible height, adjust for search bar
    var width = height * ratio; //visible width
    if (width > W) {
        width = W;
        height = width / ratio;
    }
    ...
    var top = Math.round((height - (W / pratio)) / 2);
    height = Math.round(height);
    width = Math.round(width);

    var stretch = get_pref("ytStretch"); //horizontal stretch of videos

    if (pratio > ratio && top >= 0) stretch = true; //enforce stretch for wide videos if visible area permits

    if (stretch && (ratio > 1)) //do not stretch narrow videos
        insertStyle("\
            ytd-watch-flexy[theater]:not([float]) #player-theater-container {height:" + height + "px !important; max-height:" + height + "px !important; min-height:" + height + "px !important;}\
            ytd-watch-flexy[theater]:not([float]) .html5-main-video {width:100% !important; height:auto !important; left:0px !important; top:" + top + "px !important; margin-left:0px !important;}\
            ", "ytpc_style_cinemode");
    else
        insertStyle("\
            ytd-watch-flexy[theater]:not([float]) #player-theater-container {height:" + height + "px !important; max-height:" + height + "px !important; min-height:" + height + "px !important;}\
            ytd-watch-flexy[theater]:not([float]) .html5-main-video {width:" + width + "px !important; height:" + height + "px !important; left:" + left + "px !important; margin-left:0px !important;}\
            ", "ytpc_style_cinemode");
}
...
function float(start_count) {
    ...
    //in a new video page make sure that it starts without float
    if (start_count == 0) {
        reset_float(page);
        return;
    }
...
function autoplay(start_count) {
    ...
    var cnode = docsearch("//paper-toggle-button[@id='improved-toggle']").snapshotItem(0);
    if (!cnode) cnode = docsearch("//paper-toggle-button[@id='toggle']").snapshotItem(0);
        ...
        if (!anode.getAttribute("ytpc_disable_autoplay"))
            if (cnode.getAttribute('aria-pressed') == "true") {
                if (start_count >= 5) { //give delay for first time
                    var abutton = innersearch(cnode, ".//*[@id='toggleButton']").snapshotItem(0);
                    if (abutton) {
                        simulClick(abutton);
                    }
                }
...
function auto_load(start_count) {
    ...
    var button1 = docsearch("//div[@id='related']//paper-button[contains(@class,'yt-next-continuation')" + ((start_count > 5) ? " and (not(@buttonclicked))]" : "]")).snapshotItem(0);
    if (button1) {
        if (start_count >= 5) {
            simulClick(button1);
    ...
    var button2 = docsearch("//div[@id='meta']//paper-button[(@id='more')" + ((start_count > 3) ? " and (not(@buttonclicked))]" : "]")).snapshotItem(0);
    if (button2) {
        if (start_count >= 3) {
            simulClick(button2);
...
function yt_start() {
    ...
    if (pref_ytDef != 'default' && def_count <= 2) {
